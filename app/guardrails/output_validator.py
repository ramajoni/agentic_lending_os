...
def validate_investigation_result(data: dict) -> InvestigationResult:
    ...
    try:
        ...
        log.warning("Guard 8 BLOCKED — output validation failed: %s", e.errors())
        raise OutputValidationError(e.errors())


def validate_no_discrepancy_result(data: dict) -> NoDiscrepancyResult:
    """Validate a zero-discrepancy result against the output schema."""
    try:
        result = NoDiscrepancyResult.model_validate(data)
        log.info("Guard 8 PASSED — no-discrepancy output validation OK")
        return result
    except ValidationError as e:
        log.warning("Guard 8 BLOCKED — no-discrepancy validation failed: %s", e.errors())
        raise OutputValidationError(e.errors())

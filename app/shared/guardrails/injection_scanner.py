...
def check_prompt_guard(text: str) -> tuple[bool, str | None]:
    ...
    if not hf_token or not hf_token.strip():
        # No Hugging Face token configured, rely on local heuristics
        return True, None
...
def scan_text(text: str) -> tuple[bool, str | None]:
    ...
    Hybrid scan:
      1. Fast local regex/heuristics (SQL injection, syntax exploits).
      2. Meta's Llama-Prompt-Guard-2-22M via Hugging Face (semantic jailbreaks).

    Returns:
        (is_clean, matched_pattern_or_reason)
    """
    if not text:
        return True, None
...
def scan_fields(data: dict, field_names: list[str] | None = None) -> list[dict]:
    ...
    Args:
        data: Flat dict of field values (e.g. from LoanApplication.model_dump()).
        field_names: Fields to scan. Defaults to _SCANNABLE_FIELDS.

    Returns:
        List of violation dicts: [{field, pattern, snippet}]
    """
...
def assert_no_injection(data: dict, field_names: list[str] | None = None) -> None:
    ...
    Convenience wrapper around scan_fields for use as a gate.
    """
